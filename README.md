...
- **Фильтр по кластеру** — тексты фильтруются по кластеру, назначенному разметчику.
- **Примеры интентов** — каждый пример отображается отдельной строкой в списке.
- **По умолчанию "no"** — для всех кандидатов по умолчанию выбран ответ "no".
...
- **Missed Rate** — когда интент на 2-N месте И top-1 отвергнут (no/unsure), как часто этот интент получает "yes" (высокий missed rate = модель недооценивает интент).
...
- **Высокий Top-1 Unsure** — разметчикам сложно решить.
